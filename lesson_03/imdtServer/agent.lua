...
function REQUEST:login()
	...
	player_id = skynet.call("SIMPLEDB", "lua", "login", self.name, self.password, self.modelid)

	-- 向新玩家告知他被分配到的ID
	send_request(proto_pack("login", { id = player_id, name = self.name }), client_fd)
...
function REQUEST:generic()
	print("generic  type=" .. self.type .. "   data=" .. self.data)
	broadcast_request(proto_pack("genericBC", { type = self.type, data = self.data }), nil)
end
